...
impl Loadable for TodoList {
    fn load_from_file(file_name: &str) -> io::Result<Self> {
        ...
        if path.exists() {
            ...
            let todo_list: TodoList = serde_json::from_str(&contents).unwrap();
...
impl Control for TodoList {
    ...
    fn list_tasks(&self) {
        for task in &self.tasks {
            println!("{}. {} {} [{}]", task.id, task.title, task.description, if task.completed {
                "X"
            } else {
                ""
            });
    ...
    fn list_uncompleted(&self) -> Self {
       let tasks:Vec<Task>= self.tasks.iter().filter(|t| !t.completed).cloned().collect();
     Self{
        tasks
     }
